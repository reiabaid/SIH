...
export default function Sidebar({ currentScreen, setCurrentScreen }) {
  ...
  return (
    <aside className="w-64 bg-[#1c1c1c] border-r border-[#2a2a2a] flex flex-col justify-between p-4 select-none shrink-0 z-10">
      <div>
        <div className="text-[11px] font-mono text-slate-500 uppercase tracking-wider px-3 mb-4">
          WORKFLOW PIPELINE
        </div>
          ...
          {steps.map((step) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center space-x-3 p-3 rounded-md text-left transition-colors duration-150 border-l-2 ${
                  isActive
