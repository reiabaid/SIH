...
### Similarity model — 2026-09-25 (`docs/research/offset_consistency_similarity.json`): hypothesis REJECTED
`match_tiled(model="similarity")` (2-point RANSAC, off by default, unit-tested on an exact 20 px shift) was tried on the 8 real pairs. Result: the offsets are essentially the same as the homography's (e.g. d32 x M1531872919LE rung 1: (55.5, -3023.7) m vs (55.4, -3023.7) m; d32 x M1519299970LE rung 1 -3.8 km -> -4.6 km) and still do not agree across references for the same CH2 image (d32 north: -1244, +3636, -4620, -4484, -3024...). So the disagreement is NOT caused by the 8-DOF homography extrapolating; the correspondences themselves (or the alignment feeding them) disagree. Two remaining explanations, neither testable without ground truth: (1) the candidate matches on this repetitive terrain are dominated by spurious but self-consistent sets; (2) the common-grid alignment (`align_pair`, from 4 corner coordinates) carries a real scale/rotation error that the fits are partly correcting. The translation-vote result (small shifts, tiny support) argues the strongest cluster is not where these fits put it.
NEXT (needs a person): pick 4-6 unambiguous features (crater rims) by eye in one CH2/LRO pair and compare against each candidate fit. That single measurement decides which fits, if any, are right. No further algorithm work is worth doing before it.
